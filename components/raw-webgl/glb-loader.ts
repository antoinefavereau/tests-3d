...
function parseGlbBinary(buffer: ArrayBuffer): { json: GltfJson; binary: ArrayBuffer } {
...
function readAccessor(json: GltfJson, binary: ArrayBuffer, index: number): ArrayBufferView {
...
async function imageBitmapFromBufferView(
  ...
  if (img.bufferView !== undefined) {
    ...
    const bytes = binary.slice(bv.byteOffset ?? 0, (bv.byteOffset ?? 0) + bv.byteLength);
...
export async function loadGlbModel(
  ...
  const normals   = readAccessor(json, binary, prim.attributes.NORMAL) as Float32Array;
  const texcoords = readAccessor(json, binary, prim.attributes.TEXCOORD_0) as Float32Array;
  ...
  if (prim.indices !== undefined) {
    ...
    if (acc.componentType === 5125) {
      indices = raw as Uint32Array; indexType = "UNSIGNED_INT";
    } else {
      indices = raw as Uint16Array; indexType = "UNSIGNED_SHORT";
    }
  } else {
  ...
  const resolve = (info?: { index: number }) =>
    info ? imageBitmapFromBufferView(json, binary, json.textures[info.index].source) : Promise.resolve(null);

  const [baseColor, normal, metallicRoughness, emissive, occlusion] = await Promise.all([
